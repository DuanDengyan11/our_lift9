...
namespace ego_planner
{
                                                    ...
                                                    const bool flag_polyInit)
  {
    ...
    if (flag_first_call || flag_polyInit)
    {
      ...
      /* basic params */
      Eigen::Matrix<double, 9, 3> headState, tailState;
      vector<Eigen::Vector3d> simple_path;

      headState << start_pt, start_vel, start_acc; //初始状态
      tailState << local_target_pt, local_target_vel, Eigen::Matrix<double, 9, 1>::Zero(); //目标状态
    ...
    else
    {
      ...
      Eigen::Matrix<double, 9, 3> headState, tailState;
      Eigen::MatrixXd innerPs(9, piece_nums - 1);
      ...
      tailState << local_target_pt, local_target_vel, Eigen::Matrix<double, 9, 1>::Zero();
      ...
      const bool flag_polyInit,  const bool have_local_traj)
  {
    ...
    Eigen::MatrixXd innerPts = all_pos.block(0, 1, 9, PN - 1);
    Eigen::Matrix<double, 9, 3> headState, tailState;
      ...
      const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    Eigen::Matrix<double, 9, 1> pos_sys_start, vel_sys_start, acc_sys_start;
    Eigen::Matrix<double, 9, 1> pos_sys_end, vel_sys_end, acc_sys_end;
    pos_sys_start << start_pos, cable_load_->init_coef;
    vel_sys_start << start_vel, Eigen::Matrix<double, 6, 1>::Zero();
    acc_sys_start << start_acc, Eigen::Matrix<double, 6, 1>::Zero();
    pos_sys_end << waypoints.back(), cable_load_->init_coef;
    vel_sys_end << end_vel, Eigen::Matrix<double, 6, 1>::Zero();
    acc_sys_end << end_acc, Eigen::Matrix<double, 6, 1>::Zero();
