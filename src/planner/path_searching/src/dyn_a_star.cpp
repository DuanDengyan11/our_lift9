...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt, bool use_esdf_check, double dist)
{
    ...
    while (!openSet_.empty())
    {
        ...
        ros::Time time_2 = ros::Time::now();
        if ((time_2 - time_1).toSec() > 20.0)
        {
            ROS_WARN("Failed in A star path searching !!! 20.0 seconds time limit exceeded.");
            return false;
        }
    }
...
vector<Vector3d> AStar::astarSearchAndGetSimplePath(const double step_size, Vector3d start_pt, Vector3d end_pt, double dist){
    ...
    // {
	//     grid_map_->evaluateEDT(path[i], dist_now); 
    //     cout << path[i].transpose() << " " << dist_now << endl;
    // }
    ...
    // {
	//     grid_map_->evaluateEDT(simple_path[i], dist_now); 
    //     cout << simple_path[i].transpose() << " " << dist_now << endl;
    // }
